(() => {
  ...
  var PanZoom = class {
    ...
    init() {
      ...
      if (ratio < this.port.w / this.port.h) {
    ...
    pointerMoveHandler(e) {
      ...
      } else if (Object.keys(this.pointers).length == 2) {
        let point = this.pointers[e.pointerId];
        let center = {
    ...
    zoom(e) {
      e.preventDefault();
      let delta = -Math.sign(e.deltaY), [w, h] = [this.box.width, this.box.height], [mx, my] = [e.x, e.y], dw = w * delta * this.zoomVals.step, dh = h * delta * this.zoomVals.step, [dx, dy] = [dw * mx / this.svg.clientWidth, dh * my / this.svg.clientHeight];
  ...
  function populateMap() {
    jQuery.ajax({
      dataType: "json",
      url: "wp-content/themes/africangrooves/cache/dynamic_map_data.json",
      success: function(data) {
        document.querySelectorAll("#map path").forEach((el) => {
          let key = data.states[el.id] !== void 0 ? el.id : "default";
  ...
  var title = audioPlayer.querySelector("#audio-player-main-title");
  initAudioPlayer();
  ...
  function next(element = null) {
    ...
    let next2 = tracks.querySelector(".track--future");
    let target = element ? element : next2;
    if (next2 !== null) {
      while (next2 !== target) {
        next2.classList.remove("track--future");
        next2.classList.add("track--previous");
        next2 = tracks.querySelector(".track--future");
      }
      next2.classList.remove("track--future");
      next2.classList.add("track--previous");
      let current = document.querySelector("#current-song");
      let controls = current.querySelector(".controls");
      current.innerHTML = next2.innerHTML;
      current.dataset.mp3_path = next2.dataset.mp3_path;
      current.dataset.id = next2.dataset.id;
      current.querySelector(".track-infos").appendChild(controls);
      tracks.scrollTo(0, 0);
      audio.src = next2.dataset.src;
      if (playing)
        audio.play();
    }
  }
  ...
  function update_modal(content) {
    ...
    e.innerHTML = content;
    let audio2 = e.querySelector("audio");
    if (audio2 !== null) {
  ...
  var navbar = document.querySelectorAll("#site-header a");
  navbar.forEach((e) => {
    e.addEventListener("click", (i) => {
      i.preventDefault();
      search = i.target.href;
      let modal = document.querySelector("#post-modal");
